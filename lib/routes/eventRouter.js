		...
		try {
			...
			for(let i = 0; i < expenses.length; i++) {
				...
				for(let k = 0; k < result.payements.length; k++) {
					...
					result.payements[k].amount = _round(result.payements[k].amount)
				}
			...
			for (let i = 0; i < Object.keys(bilan).length; i++) {
				...
				newBilan[user.pseudo] = _round(bilan[key]);
			}
...
router.get('/:eventId/users', 
	...
	async function(req, res, next) {
		...
		let event = await eventRepository.get(eventId)
		let users = await event.getUsers()
		return res.render('user_event/list', {
...
router.get('/:eventId/users/new', 
	...
	async function(req, res, next) {
		...
		return res.render('user_event/create', {
